...
export default function OrdineMaterialeDetail({ ordine, open, onClose, onStatusChange, canApprove = false, canEdit = false }) {
  const [noteApprovazione, setNoteApprovazione] = React.useState("");
  const [actionDialog, setActionDialog] = React.useState({ open: false, type: null });
  ...
  const handleAction = (action) => {
    ...
    if (action === "approva") {
      onStatusChange(ordine.id, "approvato");
      return;
    }
  ...
  const confirmActionWithNote = () => {
    ...
    if (actionDialog.type === "rifiuta") {
      onStatusChange(ordine.id, "rifiutato", nota);
    }

    if (actionDialog.type === "richiedi_modifica") {
      onStatusChange(ordine.id, "bozza", nota);
    }
          ...
          <ScrollArea className="flex-1 p-6">
            <div className="mb-6 bg-indigo-50 p-4 rounded-lg border border-indigo-100">
              <h4 className="text-sm font-semibold text-indigo-900 mb-2 flex items-center gap-2">
                <Building2 className="w-4 h-4" /> Societa Intestataria
              </h4>
              <div className="text-sm text-indigo-900 space-y-1">
                <p className="font-medium">{ordine.societa_intestataria_ragione_sociale || "Non selezionata"}</p>
                ...
                {ordine.societa_intestataria_indirizzo && (
                  <p>{ordine.societa_intestataria_indirizzo}</p>
                )}
                {(ordine.societa_intestataria_email || ordine.societa_intestataria_pec) && (
                  <p>
                    {ordine.societa_intestataria_email ? `Email: ${ordine.societa_intestataria_email}` : ""}
                    {ordine.societa_intestataria_email && ordine.societa_intestataria_pec ? " - " : ""}
                    {ordine.societa_intestataria_pec ? `PEC: ${ordine.societa_intestataria_pec}` : ""}
                  </p>
                )}
              </div>
            </div>
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-sm space-y-2">
                  ...
                  <div className="flex justify-between items-center border-t pt-2 mt-2">
                    ...
                    <span className="font-bold text-lg text-emerald-600">
                      EUR {Number(ordine.importo_totale || 0).toLocaleString("it-IT", { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-center font-medium text-slate-500">Q.ta</th>
          ...
          <DialogFooter className="p-4 border-t bg-slate-50 sm:justify-between items-center">
            <div className="flex items-center gap-2">
              {ordine.stato === "in_attesa_approvazione" && canApprove && (
                <>
                  <Button variant="default" className="bg-green-600 hover:bg-green-700 text-white" onClick={() => handleAction("approva")}>
          ...
          <div className="space-y-4 py-2">
            <p className="text-sm text-slate-500">
              {actionDialog.type === "richiedi_modifica"
                ? "Inserisci cosa va modificato (prodotto, quantita, prezzo o note)."
                : "Inserisci una motivazione per il rifiuto. Sara visibile al richiedente."}
            </p>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Es. Ridurre quantita articolo 2 da 50 a 30"
              />
            </div>
          </div>
          <DialogFooter>
