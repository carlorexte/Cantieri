...
const statusConfig = {
  ...
  ricevuto:               { label: "Ricevuto",            color: "bg-emerald-100 text-emerald-700",icon: PackageCheck  },
};
...
export default function OrdiniMateriali() {
  ...
  const loadOrdini = async () => {
    setLoading(true);
    try {
      const data = await supabaseDB.ordiniMateriale.getAll();
      setOrdini(data);
    } catch (error) {
  ...
  const handleCreate = async (data) => {
    try {
      await supabaseDB.ordiniMateriale.create(data);
  ...
  const handleStatusChange = async (id, newStatus, note = null) => {
    try {
      ...
      if (newStatus === 'inviato_fornitore' && !ordineCorrente?.societa_intestataria_id) {
        toast.error("Seleziona prima la società intestataria dell'ordine.");
        return;
      }

      const updateData = { stato: newStatus };
      if (note !== null) updateData.note_approvazione = note;

      await supabaseDB.ordiniMateriale.update(id, updateData);
      toast.success(`Stato aggiornato: ${statusConfig[newStatus]?.label || newStatus}`);

      setOrdini(prev => prev.map(o => o.id === id ? { ...o, ...updateData } : o));
      if (selectedOrdine?.id === id) setSelectedOrdine(prev => ({ ...prev, ...updateData }));
    } catch (error) {
  ...
  const handleRicezione = async (id, ricezioneData) => {
    try {
      await supabaseDB.ordiniMateriale.update(id, {
        stato: 'ricevuto',
        ...ricezioneData
      });
      toast.success("Merce registrata come ricevuta");
      loadOrdini();
      // Aggiorna anche il dettaglio aperto
      setSelectedOrdine(prev => prev?.id === id ? { ...prev, stato: 'ricevuto', ...ricezioneData } : prev);
    } catch (error) {
      console.error("Errore registrazione ricezione:", error);
  ...
  const groupedOrdini = React.useMemo(() => {
    ...
    filteredOrdini.forEach(order => {
      ...
      if (!groups[cantiereId]) {
        ...
        groups[cantiereId] = {
          name: cantiere ? cantiere.denominazione : (cantiereId === "unknown" ? "Senza Cantiere" : "Cantiere"),
          orders: []
        };
      }
      ...
      <div className="p-6 bg-slate-50 min-h-screen">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            ...
            {(hasPermission('ordini_materiale', 'edit') || currentUser?.role === 'admin') && (
              <Button className="shadow-sm" onClick={() => { setSelectedOrdine(null); setIsFormOpen(true); }}>
                        ...
                        >
                          <CardContent className="p-5">
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-400 border-t pt-3">
                              ...
                              <div>{ordine.dettagli_materiali?.length || 0} articoli</div>
                            </div>
                          </CardContent>
                        </Card>
                      );
          ...
          <OrdineMaterialeDetail
            ...
            onRicezione={handleRicezione}
            canApprove={hasPermission('ordini_materiale', 'accept') || currentUser?.role === 'admin'}
            canEdit={hasPermission('ordini_materiale', 'edit') || currentUser?.role === 'admin'}
          />
        </div>
      </div>
    </PermissionGuard>
