...
export default function ProfiloAziendaPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const aziendeData = await Azienda.list();
      ...
      if (currentAzienda.id) {
        const documentiData = await Documento.filter({ 
  ...
  const handleSaveAzienda = async () => {
    ...
    try {
      if (azienda.id) {
        await Azienda.update(azienda.id, azienda);
        toast.success("Profilo aziendale aggiornato.");
      } else {
        const newAzienda = await Azienda.create(azienda);
  ...
  const handleDocumentoSubmit = async (documentoData) => {
    try {
      if (editingDocumento) {
        await Documento.update(editingDocumento.id, documentoData);
